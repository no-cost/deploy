...
- [`apps/*.conf`](./apps/) - contains configuration files for `location @<service_type>` rules. When we figure out which `service_type` a site has, we can route it dynamically to the correct `location` block (correct config for that service).
